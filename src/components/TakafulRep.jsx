...
function TakafulRep() {
    ...
    const [allTakafulReports, setAllTakafulReports] = useState([]);
    ...
    const tableHeaders = useMemo(() => [
        { key: 'reportIdentifier', label: t('takaful.table.reportIdentifier', 'Policy/Report No.') },
        { key: 'accidentDate', label: t('takaful.table.accidentDate', 'Accident Date'), type: 'date' },
        { key: 'accidentLocation', label: t('takaful.table.location', 'Location') },
        { key: 'insuredName', label: t('takaful.table.insuredName', 'Insured Name') },
        { key: 'driverName', label: t('takaful.table.driverName', 'Driver Name') },
        { key: 'vehiclePlateNo', label: t('takaful.table.vehiclePlateNo', 'Vehicle Plate No.') },
        { key: 'agentName', label: t('takaful.table.agentName', 'Agent Name') },
        { key: 'actions', label: t('takaful.table.actions', 'Actions'), align: language === 'ar' ? 'left' : 'right' },
    ], [t, language]);
    ...
    const fetchTakafulReports = async () => {
        ...
        try {
            ...
            const res = await axios.get(`http://localhost:3002/api/v1/TakafulAccidentReport/all`, { headers: { token } });
    ...
    const sortedData = useMemo(() => {
        let sortableItems = [...allTakafulReports];
        if (sortConfig.key) {
    ...
    }, [allTakafulReports, sortConfig]);

    const filteredTakafulReports = useMemo(() => {
    ...
    const visibleReports = useMemo(() => filteredTakafulReports.slice(0, displayCount), [filteredTakafulReports, displayCount]);

    const exportToExcel = () => {
        if (filteredTakafulReports.length === 0) { toast.info(t('common.noDataToExport')); return; }
        const exportData = filteredTakafulReports.map(r => ({
            [t('takaful.table.reportIdentifier')]: r.reportIdentifier,
            [t('takaful.table.accidentDate')]: r.accidentDate,
            [t('takaful.table.location')]: r.accidentLocation,
            [t('takaful.table.insuredName')]: r.insuredName,
            [t('takaful.table.driverName')]: r.driverName,
            [t('takaful.table.vehiclePlateNo')]: r.vehiclePlateNo,
            [t('takaful.table.agentName')]: r.agentName,
        }));
        const worksheet = XLSX.utils.json_to_sheet(exportData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, t('takaful.exportSheetName', "Takaful Reports"));
        XLSX.writeFile(workbook, t('takaful.exportExcelFileName', "takaful_reports.xlsx"));
    };
    ...
    const exportToCSV = () => {
        ...
        const csvRows = [
            headers.join(','),
            ...filteredTakafulReports.map(r =>
                [
                    `"${(r.reportIdentifier || '').toString().replace(/"/g, '""')}"`,
                    `"${(r.accidentDate || '').toString().replace(/"/g, '""')}"`,
                    `"${(r.accidentLocation || '').toString().replace(/"/g, '""')}"`,
                    `"${(r.insuredName || '').toString().replace(/"/g, '""')}"`,
                    `"${(r.driverName || '').toString().replace(/"/g, '""')}"`,
                    `"${(r.vehiclePlateNo || '').toString().replace(/"/g, '""')}"`,
                    `"${(r.agentName || '').toString().replace(/"/g, '""')}"`
                ].join(',')
            )
        ];
        ...
        link.setAttribute('download', 'takaful_reports.csv');
    ...
    const exportToPDF = () => {
        ...
        const doc = new jsPDF();
        doc.setFontSize(14);
        doc.text(t('takaful.exportPdfTitle', "Takaful Accident Reports"), 14, 15);
        const exportColumns = tableHeaders.filter(h => h.key !== 'actions').map(h => ({ header: h.label, dataKey: h.key }));
        doc.autoTable({
            columns: exportColumns,
            body: filteredTakafulReports,
            startY: 22, styles: { fontSize: 8, font: "Arial" },
            headStyles: { fillColor: [0, 128, 128], textColor: 255 },
        });
        doc.save(t('takaful.exportPdfFileName', "takaful_reports.pdf"));
    };

    const handlePrint = () => {
        if (filteredTakafulReports.length === 0) { toast.info(t('common.noDataToExport')); return; }
        const printWindow = window.open('', '_blank');
        const title = t('takaful.printReportTitle', 'Takaful Accident Reports');
        ...
        filteredTakafulReports.forEach(report => {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm(t('takaful.confirmDelete', "Are you sure you want to delete this report?"))) return;
        const token = `islam__${localStorage.getItem("token")}`;
        try {
    ...
    const handleAddReportSuccess = () => { setShowAddReportForm(false); fetchTakafulReports(); };
    useEffect(() => { fetchTakafulReports(); }, [language]);

    const handleScroll = useCallback(() => {
        const nearBottom = window.innerHeight + document.documentElement.scrollTop >= document.documentElement.offsetHeight - 200;
        if (nearBottom && displayCount < filteredTakafulReports.length && !isLoadingMore && !loading) {
            setIsLoadingMore(true);
            setTimeout(() => {
                setDisplayCount(prevCount => Math.min(prevCount + ROWS_PER_PAGE, filteredTakafulReports.length));
                setIsLoadingMore(false);
            }, 300);
        }
    }, [displayCount, filteredTakafulReports.length, isLoadingMore, loading]);
        ...
        <div className="px-4 py-5 dark:bg-dark2 dark:text-dark3 min-h-screen">
            <div className="bg-[rgb(255,255,255)] dark:bg-navbarBack flex p-[22px] rounded-md justify-between items-center mb-3" dir={language === "ar" ? "rtl" : "ltr"}>
                <div className="flex gap-[14px] items-center text-gray-700 dark:text-gray-300">
                    ...
                    <span className="text-gray-500 dark:text-gray-400">{t('navigation.takafulReport', 'Takaful Report')}</span>
                </div>
            ...
            <div className='flex rounded-md justify-between items-start flex-wrap my-2'>
                <div className="flex gap-2 flex-wrap">
                    <Button onClick={exportToCSV} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }}  size="small" disabled={filteredTakafulReports.length === 0}>{t('buttons.exportCsv', 'CSV')}</Button>
                    <Button onClick={exportToExcel} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }}  size="small" disabled={filteredTakafulReports.length === 0}>{t('buttons.exportExcel', 'Excel')}</Button>
                    <Button onClick={exportToPDF} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }}  size="small" disabled={filteredTakafulReports.length === 0}>{t('buttons.exportPdf', 'PDF')}</Button>
                    <Button onClick={handlePrint} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }} size="small" disabled={filteredTakafulReports.length === 0}>{t('buttons.print', 'Print')}</Button>
                </div><div className="">
            ...
            <div className={`mb-4 text-sm text-gray-600 dark:text-gray-400 ${language === 'ar' ? 'text-right' : 'text-left'}`}>
                {t('takaful.showing_results', 'Showing {{count}} of {{total}} reports', { count: visibleReports.length, total: filteredTakafulReports.length })}
            </div>
                        ...
                        {loading && visibleReports.length === 0 ? (
                            ...
                            visibleReports.map((report) => (
                                <tr key={report.id} className="bg-[rgb(255,255,255)] dark:bg-navbarBack border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">{report.vehiclePlateNo}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">{report.agentName}</td>
                            ...
                            <tr><td colSpan={tableHeaders.length} className="text-center py-16 text-gray-500 dark:text-gray-400">{searchText ? t('common.noSearchResults') : t('takaful.noReportsFound')}</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            {!isLoadingMore && !loading && displayCount >= filteredTakafulReports.length && filteredTakafulReports.length > ROWS_PER_PAGE && <div className="text-center py-8 text-gray-500 dark:text-gray-400">{t('common.endOfResults')}</div>}
            {showAddReportForm && <InsuranceTakafulRep isOpen={showAddReportForm} onClose={() => setShowAddReportForm(false)} onReportAdded={handleAddReportSuccess} />}
        </div>
    );
}
