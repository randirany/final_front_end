...
function AhlieReport() {
    ...
    const [allAhliaReports, setAllAhliaReports] = useState([]);
    ...
    const tableHeaders = useMemo(() => [
        { key: 'reportNumber', label: t('ahliaReport.table.reportNo', 'Report No.') },
        { key: 'accidentDate', label: t('ahliaReport.table.date', 'Accident Date'), type: 'date' },
        { key: 'accidentLocation', label: t('ahliaReport.table.location', 'Location') },
        { key: 'policyNumber', label: t('ahliaReport.table.policyNo', 'Policy No.') },
        { key: 'insuredName', label: t('ahliaReport.table.insured', 'Insured Name') },
        { key: 'driverName', label: t('ahliaReport.table.driver', 'Driver Name') },
        { key: 'vehicleRegNo', label: t('ahliaReport.table.vehicle', 'Vehicle Reg. No.') },
        { key: 'actions', label: t('ahliaReport.table.actions', 'Actions'), align: language === 'ar' ? 'left' : 'right' },
    ], [t, language]);
    ...
    const fetchAhliaReports = async () => {
        ...
        try {
            ...
            const res = await axios.get(`http://localhost:3002/api/v1/AhliaAccidentReport/all`, { headers: { token } });
    ...
    const sortedData = useMemo(() => {
        let sortableItems = [...allAhliaReports];
        if (sortConfig.key) {
    ...
    }, [allAhliaReports, sortConfig]);

    const filteredAhliaReports = useMemo(() => {
    ...
    const visibleReports = useMemo(() => filteredAhliaReports.slice(0, displayCount), [filteredAhliaReports, displayCount]);

    const exportToExcel = () => {
        if (filteredAhliaReports.length === 0) { toast.info(t('common.noDataToExport')); return; }
        const exportData = filteredAhliaReports.map(report => ({
            [t('ahliaReport.table.reportNo')]: report.reportNumber,
            [t('ahliaReport.table.date')]: report.accidentDate,
            [t('ahliaReport.table.location')]: report.accidentLocation,
            [t('ahliaReport.table.policyNo')]: report.policyNumber,
            [t('ahliaReport.table.insured')]: report.insuredName,
            [t('ahliaReport.table.driver')]: report.driverName,
            [t('ahliaReport.table.vehicle')]: report.vehicleRegNo,
        }));
        const worksheet = XLSX.utils.json_to_sheet(exportData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, t('ahliaReport.exportSheetName', 'AhliaReports'));
        XLSX.writeFile(workbook, t('ahliaReport.exportExcelFileName', 'ahlia_reports.xlsx'));
    };
    ...
    const exportToCSV = () => {
        ...
        const csvRows = [
            headers.join(','),
            ...filteredAhliaReports.map(report =>
                [
                    `"${(report.reportNumber || '').toString().replace(/"/g, '""')}"`,
                    `"${(report.accidentDate || '').toString().replace(/"/g, '""')}"`,
                    `"${(report.accidentLocation || '').toString().replace(/"/g, '""')}"`,
                    `"${(report.policyNumber || '').toString().replace(/"/g, '""')}"`,
                    `"${(report.insuredName || '').toString().replace(/"/g, '""')}"`,
                    `"${(report.driverName || '').toString().replace(/"/g, '""')}"`,
                    `"${(report.vehicleRegNo || '').toString().replace(/"/g, '""')}"`
                ].join(',')
            )
        ];
        ...
        link.setAttribute('download', 'ahlia_reports.csv');
    ...
    const exportToPDF = () => {
        ...
        const doc = new jsPDF();
        doc.setFontSize(14);
        doc.text(t('ahliaReport.exportPdfTitle', 'Al-Ahlia Accident Reports'), 14, 15);
        const exportColumns = tableHeaders.filter(h => h.key !== 'actions').map(h => ({ header: h.label, dataKey: h.key }));
        doc.autoTable({
            columns: exportColumns,
            body: filteredAhliaReports,
            startY: 22, styles: { fontSize: 8, font: "Arial" },
            headStyles: { fillColor: [41, 128, 185], textColor: 255 },
        });
        doc.save(t('ahliaReport.exportPdfFileName', 'ahlia_reports.pdf'));
    };
    ...
    const handleDelete = async (id) => {
        if (!window.confirm(t('ahliaReport.deleteConfirm', "Are you sure you want to delete this report?"))) return;
        const token = `islam__${localStorage.getItem("token")}`;
        try {
    ...
    const handleAddReportSuccess = () => { setShowAddReportForm(false); fetchAhliaReports(); };

    useEffect(() => { fetchAhliaReports(); }, [language]);

    const handleScroll = useCallback(() => {
        const nearBottom = window.innerHeight + document.documentElement.scrollTop >= document.documentElement.offsetHeight - 200;
        if (nearBottom && displayCount < filteredAhliaReports.length && !isLoadingMore && !loading) {
            setIsLoadingMore(true);
            setTimeout(() => {
                setDisplayCount(prevCount => Math.min(prevCount + ROWS_PER_PAGE, filteredAhliaReports.length));
                setIsLoadingMore(false);
            }, 300);
        }
    }, [displayCount, filteredAhliaReports.length, isLoadingMore, loading]);
        ...
        <div className="px-4 py-5 dark:bg-dark2 dark:text-dark3 min-h-screen">
            <div className="bg-[rgb(255,255,255)] dark:bg-navbarBack flex p-[22px] rounded-md justify-between items-center mb-3" dir={language === "ar" ? "rtl" : "ltr"}>
                <div className="flex gap-[14px] items-center text-gray-700 dark:text-gray-300">
                    ...
                    <span className="text-gray-500 dark:text-gray-400">{t('navigation.ahliaReport', 'Al-Ahlia Accident Reports')}</span>
                </div>
            ...
            <div className='flex rounded-md justify-between items-start flex-wrap my-2'>
                 <div className="flex gap-2 flex-wrap">
                    <Button onClick={exportToCSV} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }}  size="small" disabled={filteredAhliaReports.length === 0}>{t('buttons.exportCsv', 'CSV')}</Button>
                    <Button onClick={exportToExcel} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }}  size="small" disabled={filteredAhliaReports.length === 0}>{t('buttons.exportExcel', 'Excel')}</Button>
                    <Button onClick={exportToPDF} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }}  size="small" disabled={filteredAhliaReports.length === 0}>{t('buttons.exportPdf', 'PDF')}</Button>
                    <Button onClick={handlePrint} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }} size="small" disabled={filteredAhliaReports.length === 0}>{t('buttons.print', 'Print')}</Button>
                </div><div className="">
                    <input type="text" placeholder={t('common.searchPlaceholder', "Search in Report ...")} className={`p-2 border rounded-md mb-4 dark:bg-gray-700 w-[300px] dark:text-[rgb(255,255,255)] dark:!border-none ${language === 'ar' ? 'text-right' : 'text-left'}`} value={searchText} onChange={(e) => setSearchText(e.target.value)} />
                </div>
               
            </div>

            <div className={`mb-4 text-sm text-gray-600 dark:text-gray-400 ${language === 'ar' ? 'text-right' : 'text-left'}`}>
                {t('ahliaReport.showing_results', 'Showing {{count}} of {{total}} reports', { count: visibleReports.length, total: filteredAhliaReports.length })}
            </div>
                        ...
                        {loading && visibleReports.length === 0 ? (
                            ...
                            visibleReports.map((report) => (
                                <tr key={report.id} className="bg-[rgb(255,255,255)] dark:bg-navbarBack border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                    <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-[rgb(255,255,255)]">{report.reportNumber}</td>
                            ...
                            <tr><td colSpan={tableHeaders.length} className="text-center py-16 text-gray-500 dark:text-gray-400">{searchText ? t('common.noSearchResults') : t('ahliaReport.noReportsFound')}</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            {!isLoadingMore && !loading && displayCount >= filteredAhliaReports.length && filteredAhliaReports.length > ROWS_PER_PAGE && <div className="text-center py-8 text-gray-500 dark:text-gray-400">{t('common.endOfResults', "You've reached the end of the results")}</div>}

            {showAddReportForm && <InsuranceAhliaRep isOpen={showAddReportForm} onClose={() => setShowAddReportForm(false)} onReportAdded={handleAddReportSuccess} />}
        </div>
    );
}
