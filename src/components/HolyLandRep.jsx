...
function HolyLandRep() {
    ...
    const [allHoliReports, setAllHoliReports] = useState([]);
    ...
    const tableHeaders = useMemo(() => [
        { key: 'reportIdDisplay', label: t('holyLand.table.reportIdDisplay', "Report ID") },
        { key: 'policyNumber', label: t('holyLand.table.policyNumber', "Policy No.") },
        { key: 'accidentDate', label: t('holyLand.table.accidentDate', "Accident Date"), type: 'date' },
        { key: 'accidentLocation', label: t('holyLand.table.location', 'Location') },
        { key: 'insuredName', label: t('holyLand.table.insuredName', 'Insured Name') },
        { key: 'driverName', label: t('holyLand.table.driverName', "Driver Name") },
        { key: 'vehiclePlateNumber', label: t('holyLand.table.vehiclePlateNumber', "Vehicle Plate No.") },
        { key: 'actions', label: t('holyLand.table.actions', "Actions"), align: (language === 'ar' || language === 'he') ? 'left' : 'right' },
    ], [t, language]);
    ...
    const fetchHoliReports = useCallback(async () => {
        setLoading(true);
        try {
            const token = `islam__${localStorage.getItem("token")}`;
            const res = await axios.get(`http://localhost:3002/api/v1/HolyLand/all`, { headers: { token } });
            ...
            const formattedData = reports.map(report => {
                let insuredName = 'N/A';
                if (report.insuredId && typeof report.insuredId === 'object' && report.insuredId.first_name) {
                    insuredName = `${report.insuredId.first_name} ${report.insuredId.last_name || ''}`.trim();
                } else if (report.ownerAndDriverDetails?.ownerName) {
                    insuredName = report.ownerAndDriverDetails.ownerName;
                }
                return {
                    id: report._id,
                    ...
                    vehiclePlateNumber: report.vehicleDetails?.plateNumber || report.insuranceDetails?.vehicleNumber || 'N/A',
    ...
    const sortedData = useMemo(() => {
        let sortableItems = [...allHoliReports];
        if (sortConfig.key) {
    ...
    }, [allHoliReports, sortConfig]);

    const filteredHoliReports = useMemo(() => {
    ...
    const visibleReports = useMemo(() => filteredHoliReports.slice(0, displayCount), [filteredHoliReports, displayCount]);

    const handleExportExcel = () => {
        if (filteredHoliReports.length === 0) { toast.info(t('common.noDataToExport')); return; }
        const exportData = filteredHoliReports.map(row => ({
            [t('holyLand.table.reportIdDisplay')]: row.reportIdDisplay,
            [t('holyLand.table.policyNumber')]: row.policyNumber,
            [t('holyLand.table.accidentDate')]: row.accidentDate,
            [t('holyLand.table.location')]: row.accidentLocation,
            [t('holyLand.table.insuredName')]: row.insuredName,
            [t('holyLand.table.driverName')]: row.driverName,
            [t('holyLand.table.vehiclePlateNumber')]: row.vehiclePlateNumber,
        }));
        const worksheet = XLSX.utils.json_to_sheet(exportData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, t('holyLand.exportSheetName', "Holy Land Reports"));
        XLSX.writeFile(workbook, t('holyLand.exportExcelFileName', "Holy_Land_Reports.xlsx"));
    };
    ...
    const handleExportCSV = () => {
        ...
        const csvRows = [
            headers.join(','),
            ...filteredHoliReports.map(row => 
                [
                    `"${(row.reportIdDisplay || '').toString().replace(/"/g, '""')}"`,
                    `"${(row.policyNumber || '').toString().replace(/"/g, '""')}"`,
                    ...
                    `"${(row.vehiclePlateNumber || '').toString().replace(/"/g, '""')}"`,
                ].join(',')
            )
        ];
        ...
        link.setAttribute('download', 'holy_land_reports.csv');
    ...
    const handleExportPDF = () => {
        ...
        const doc = new jsPDF();
        doc.setFontSize(14);
        doc.text(t('holyLand.exportPdfTitle', "Holy Land Accident Reports"), 14, 15);
        const exportColumns = tableHeaders.filter(h => h.key !== 'actions').map(h => ({ header: h.label, dataKey: h.key }));
        doc.autoTable({
            columns: exportColumns,
            body: filteredHoliReports,
            startY: 22,
            styles: { fontSize: 8, font: "Arial" },
            headStyles: { fillColor: [139, 0, 0], textColor: 255 },
        });
        doc.save(t('holyLand.exportPdfFileName', "Holy_Land_Reports.pdf"));
    };
    
    const handlePrint = () => {
        if (filteredHoliReports.length === 0) { toast.info(t('common.noDataToExport')); return; }
        const printWindow = window.open('', '_blank');
        const title = t('holyLand.printReportTitle', 'Holy Land Accident Reports');
        ...
        filteredHoliReports.forEach(report => {
            printWindow.document.write(`<tr>
                <td>${report.reportIdDisplay || ''}</td>
                ...
                <td>${report.vehiclePlateNumber || ''}</td>
            </tr>`);
        });
    ...
    const handleDelete = async (reportIdToDelete) => {
        if (!window.confirm(t('holyLand.deleteConfirm'))) return;
        const token = `islam__${localStorage.getItem("token")}`;
        try {
    ...
    useEffect(() => { fetchHoliReports(); }, [fetchHoliReports]);
    
    const handleScroll = useCallback(() => {
        const nearBottom = window.innerHeight + document.documentElement.scrollTop >= document.documentElement.offsetHeight - 200;
        if (nearBottom && displayCount < filteredHoliReports.length && !isLoadingMore && !loading) {
            setIsLoadingMore(true);
            setTimeout(() => {
                setDisplayCount(prevCount => Math.min(prevCount + ROWS_PER_PAGE, filteredHoliReports.length));
                setIsLoadingMore(false);
            }, 300);
        }
    }, [displayCount, filteredHoliReports.length, isLoadingMore, loading]);
        ...
        <div className="px-4 py-5 dark:bg-dark2 dark:text-dark3 min-h-screen">
            <div className="bg-[rgb(255,255,255)] dark:bg-navbarBack flex p-[22px] rounded-md justify-between items-center mb-3" dir={(language === "ar" || language === "he") ? "rtl" : "ltr"}>
                <div className="flex items-center gap-2 text-sm md:text-base text-gray-700 dark:text-gray-300">
                    ...
                    <span className="text-gray-500 dark:text-gray-400">{t('holyLand.reportsTitle')}</span>
                </div>
                <div className="flex flex-wrap gap-2">
                    <Button onClick={() => setShowAddReportForm(true)} variant="contained" sx={{ background: '#6C5FFC', color: '#fff' }} startIcon={<AddIcon />} size="small">{t('holyLand.addReportButton')}</Button>
                </div>
            </div>
            <div className='flex rounded-md justify-between items-start flex-wrap my-2'>
                <div className="flex gap-2 flex-wrap">
                    <Button onClick={handleExportCSV} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }} size="small" disabled={filteredHoliReports.length === 0}>{t('buttons.exportCsv', 'CSV')}</Button>
                    <Button onClick={handleExportExcel} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }} size="small" disabled={filteredHoliReports.length === 0}>{t('buttons.exportExcel', 'Excel')}</Button>
                    <Button onClick={handleExportPDF} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }} size="small" disabled={filteredHoliReports.length === 0}>{t('buttons.exportPdf', 'PDF')}</Button>
                    <Button onClick={handlePrint} variant="outlined" sx={{ background: '#6C5FFC', color: '#fff' }} size="small" disabled={filteredHoliReports.length === 0}>{t('buttons.print', 'Print')}</Button>
                </div><div className="">
                    <input type="text" placeholder={t('common.searchPlaceholder', "Search in Report ...")} className={`p-2 border rounded-md w-[300px] mb-2 dark:bg-gray-700 dark:text-[rgb(255,255,255)] dark:!border-none ${(language === 'ar' || language === 'he') ? 'text-right' : 'text-left'}`} value={searchText} onChange={(e) => setSearchText(e.target.value)} />
                </div>

            </div>
            <div className={`mb-4 text-sm text-gray-600 dark:text-gray-400 ${(language === 'ar' || language === 'he') ? 'text-right' : 'text-left'}`}>
                {t('holyLand.showing_results', { count: visibleReports.length, total: filteredHoliReports.length })}
            </div>
                        ...
                        {loading && visibleReports.length === 0 ? (
                            ...
                            visibleReports.map((report) => (
                                <tr key={report.id} className="bg-[rgb(255,255,255)] dark:bg-navbarBack border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                                    <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-[rgb(255,255,255)]">{report.reportIdDisplay}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">{report.policyNumber}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">{report.accidentDate}</td>
                                    <td className="px-6 py-4 whitespace-nowrap max-w-xs truncate" title={report.accidentLocation}>{report.accidentLocation}</td>
                                    <td className="px-6 py-4 whitespace-nowrap max-w-xs truncate" title={report.insuredName}>{report.insuredName}</td>
                                    <td className="px-6 py-4 whitespace-nowrap max-w-xs truncate" title={report.driverName}>{report.driverName}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">{report.vehiclePlateNumber}</td>
                                    <td className={`px-6 py-4 ${(language === 'ar' || language === 'he') ? 'text-left' : 'text-right'}`}>
                                        ...
                                        <Menu anchorEl={anchorEls[report.id]} open={Boolean(anchorEls[report.id])} onClose={() => handleMenuClose(report.id)} anchorOrigin={{ vertical: 'bottom', horizontal: (language === 'ar' || language === 'he') ? 'left' : 'right' }} transformOrigin={{ vertical: 'top', horizontal: (language === 'ar' || language === 'he') ? 'left' : 'right' }} MenuListProps={{ className: 'dark:bg-gray-800 dark:text-gray-200' }}>
                                            <MenuItem onClick={() => handleDelete(report.id)} className="dark:hover:bg-gray-700 text-red-600 dark:text-red-400"><DeleteIcon fontSize="small" sx={{ mr: 1 }} /> {t("common.delete")}</MenuItem>
                                        </Menu>
                                    </td>
                                </tr>
                            ))
                        ) : (
                            <tr><td colSpan={tableHeaders.length} className="text-center py-16 text-gray-500 dark:text-gray-400">{searchText ? t('common.noSearchResults') : t('holyLand.noReportsFound')}</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            {!isLoadingMore && !loading && displayCount >= filteredHoliReports.length && filteredHoliReports.length > ROWS_PER_PAGE && <div className="text-center py-8 text-gray-500 dark:text-gray-400">{t('common.endOfResults')}</div>}
            {showAddReportForm && <InsuranceHoliRep isOpen={showAddReportForm} onClose={(success) => { setShowAddReportForm(false); if (success === true) { fetchHoliReports(); } }} />}
        </div>
    );
}
