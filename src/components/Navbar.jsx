  ...
  useEffect(() => {
    if (tokenForAPI) {
      try {
        ...
        console.log('Decoded JWT (uswrDate):', decoded);
      } catch (e) {
        console.error("Navbar: Error decoding token:", e);
      }
      console.log('Token check successful (yesss)'); 
      if (UserData?._id) {
        fetchNotifications();
        ...
        socket.on('connect', () => {
          ...
          if (UserData?._id) {
            ...
            console.warn('Socket connected, but UserData._id not available for registration.');
          }
        });
        ...
        console.log("Token exists, but UserData._id not available. Socket not initialized/connected.");
      ...
      if (socket && socket.connected) {
        console.log('User logged out or token invalid, disconnecting socket.');
        socket.disconnect();
      }
      setNotifications([]);
      setUnreadCount(0);
...
function Navbar({ setSidebarOpen, sidebarOpen }) {
  ...
  useEffect(() => {
    ...
    if (!UserData?.id) {
      console.log("[useEffect] No UserData.id, exiting effect");
      return;
    }

    const rawToken = localStorage.getItem("token");
    console.log("[useEffect] rawToken:", rawToken);
    if (!rawToken || rawToken === "null" || rawToken === "undefined") {
      console.log("[useEffect] No valid token, exiting effect");
      return;
    }
    const token = `islam__${rawToken}`;
    console.log("[useEffect] Token for API:", token);
    ...
    const fetchNotifications = async () => {
      ...
      try {
        ...
        setNotifications(fetchedNotifications);
        setUnreadCount(fetchedNotifications.filter(n => !n.isRead).length);
        console.log("[fetchNotifications] Notifications loaded, unreadCount:", unreadCount);
      } catch (error) {
    ...
    socket.on('connect', () => {
      ...
      console.log('📡 Registered user on socket:', UserData.id);
    });

    socket.on('newNotification', (newNotification) => {
      console.log('📥 New notification via socket:', newNotification);
      setNotifications(prev => [newNotification, ...prev]);
      if (!newNotification.isRead) {
        setUnreadCount(prev => prev + 1);
      }
  ...
  const handleMarkAsRead = async (notificationId) => {
    console.log("[handleMarkAsRead] notificationId:", notificationId);

    const rawToken = localStorage.getItem("token");
    if (!rawToken) {
      console.log("[handleMarkAsRead] No token found, aborting");
      return;
    }
    const token = `islam__${rawToken}`;
    ...
    if (notificationToUpdate && !notificationToUpdate.isRead) {
      ...
      setUnreadCount(prev => (prev > 0 ? prev - 1 : 0));
      console.log("[handleMarkAsRead] Marked locally as read");
    } else {
      console.log("[handleMarkAsRead] Notification already read or not found");
      return;
    }
    ...
    try {
      ...
      console.log("[handleMarkAsRead] API mark as read success");
    } catch (error) {
  ...
  const handleMarkAllAsRead = async () => {
    ...
    if (!rawToken || unreadCount === 0) {
      console.log("[handleMarkAllAsRead] No token or no unread notifications, aborting");
      return;
    }
    const token = `islam__${rawToken}`;
    ...
    try {
      ...
      console.log("[handleMarkAllAsRead] API mark all as read success");
    } catch (error) {
      console.error("[handleMarkAllAsRead] Error marking all notifications as read:", error);
      setNotifications(originalNotifications);
      setUnreadCount(oldUnreadCount);
      alert(t('nav.dropdown.markAllReadError', 'Failed to mark all as read.'));
    }
  ...
  const toggleNotification = () => {
    ...
    if (profileOpen) setProfileOpen(false);
    console.log("[toggleNotification] notificationOpen:", !notificationOpen);
  };

  const toggleProfile = () => {
    setProfileOpen(prev => !prev);
    if (notificationOpen) setNotificationOpen(false);
    console.log("[toggleProfile] profileOpen:", !profileOpen);
  };

  const handleThemeToggle = () => {
    toggleDarkMode();
    console.log("[handleThemeToggle] Dark mode toggled");
  };
                    ...
                    {notifications.length > 0 && ( 
                      <div className="flex flex-col gap-2">
                        {unreadCount > 0 && ( 
                         ...
                         {/* For example:
                         <button
                            onClick={() => { console.log("Navigate to all notifications page"); setNotificationOpen(false);}}
