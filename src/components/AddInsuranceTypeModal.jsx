...
const AddInsuranceTypeModal = ({ isOpen, onClose, onInsuranceTypeAdded }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={handleBackdropClick}>
      <div className="w-full max-w-lg bg-white rounded-lg shadow-xl dark:bg-navbarBack max-h-[95vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
          <h2 className="text-2xl font-semibold dark:text-white">{t('insuranceType.addInsuranceType', 'Add Insurance Type')}</h2>
          <button
        ...
        <form className="flex-1 overflow-y-auto" onSubmit={handleSubmit}>
          <div className="p-6 space-y-6">
            ...
            {/* Pricing Type */}
            <div>
              <label htmlFor="pricing_type_id" className="block text-sm font-medium dark:text-gray-300 mb-1">
              ...
              <select
                ...
                onChange={handleInputChange}
                className={`w-full p-2.5 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${errors.pricing_type_id ? 'border-red-500' : ''}`}
              >
                <option value="">{t('insuranceType.placeholders.selectPricingType', 'Select pricing type')}</option>
                {pricingTypes.map((type) => (
                  <option key={type._id} value={type._id}>
                    {type.name} {type.requiresPricingTable ? '(Matrix-based)' : '(Simple)'}
                  </option>
                ))}
              </select>
              {errors.pricing_type_id && <p className="mt-1 text-sm text-red-500">{errors.pricing_type_id}</p>}
          ...
          <div className="flex gap-3 justify-end px-6 py-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            ...
            <button
              ...
              {loading ? t('common.saving', 'Saving...') : t('insuranceType.buttons.save', 'Save')}
            </button>
          </div>
        </form>
      </div>
