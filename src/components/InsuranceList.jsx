...
function InsuranceList() {
    ...
    const formatDateForDisplay = (date) => {
        ...
        return new Date(date).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US', { calendar: 'gregory' });
    };
                                ...
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-300">
                                    <tr>
                                        {tableColumns.map(col => (
                                            <th key={col.key} scope="col" className={`px-4 py-3 ${col.align === 'right' ? 'text-right' : (col.align === 'left' ? 'text-left' : (language === 'ar' ? 'text-right' : 'text-left'))} ${col.key !== 'actions' ? 'cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600' : ''}`} onClick={() => col.key !== 'actions' && requestSort(col.key)}>
