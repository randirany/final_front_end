...
function AddInsuranceMandatory({ onClose, isOpen, vehicleId, insuredId, onInsuranceAdded }) {
    ...
    const handleSubmit = async (e) => {
        ...
        // --- Validation ---
        if (!formData.insuranceType) return setApiMessage({ text: t("insurance.validation.typeRequired", "Insurance Type required"), type: "error" });
        if (!formData.insuranceCompany) return setApiMessage({ text: t("insurance.validation.companyRequired", "Insurance Company required"), type: "error" });

        if (!formData.paymentMethod) return setApiMessage({ text: t("insurance.validation.paymentMethodRequired", "Payment Method required"), type: "error" });
        if (!formData.priceisOnTheCustomer) return setApiMessage({ text: t("insurance.validation.priceRequired", "Price on customer required"), type: "error" });
        if (!formData.paidAmount) return setApiMessage({ text: t("insurance.validation.paidAmountRequired", "Paid Amount required"), type: "error" });
       // if (!insuredId || !vehicleId) return setApiMessage({ text: t("insurance.validation.missingIds", "Missing insured or vehicle ID"), type: "error" });
        ...
        try {
            ...
            setApiMessage({ text: res.data.message || t("insurance.messages.addSuccess", "Insurance added successfully"), type: "success" });
    ...
    const paymentMethodOptions = [
        ...
        { value: "check", label: t("insurance.paymentMethods.check", "Check") },
        { value: "bank_transfer", label: t("insurance.paymentMethods.bankTransfer", "Bank Transfer") },
    ];

    const insuranceTypeOptions = [
        { value: "compulsory", label: t("insurance.types.compulsory", "Compulsory") },
        { value: "comprehensive", label: t("insurance.types.comprehensive", "Comprehensive") },
    ];
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3 backdrop-blur-sm" onClick={() => { if (!loading) onClose(); }}>
            <div className="w-full max-w-2xl bg-white rounded-lg shadow-xl p-6 dark:bg-navbarBack h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between pb-3 border-b dark:border-gray-700 mb-4 sticky top-0 bg-white dark:bg-navbarBack z-10 px-0 pt-0">
                    <h2 className="text-xl font-semibold dark:text-white">{t("insurance.addInsurance", "Add New Insurance")}</h2>
                ...
                <div className="overflow-y-auto hide-scrollbar flex-grow">
                    <form onSubmit={handleSubmit} className="space-y-4 pr-1">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {/* Insurance Type */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("insurance.labels.insuranceType", "Insurance Type")} <span className="text-red-500">*</span></label>
                            ...
                            {/* Insurance Company */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("insurance.labels.insuranceCompany", "Insurance Company")} <span className="text-red-500">*</span></label>
                                <select name="insuranceCompany" value={formData.insuranceCompany} onChange={handleInputChange} className="mt-1 w-full p-2.5 border rounded-md dark:bg-gray-700 dark:text-white">
                                    <option value="">{t("insurance.placeholders.chooseCompany", "Choose Company")}</option>
                            ...
                            {/* Agent */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("insurance.labels.agent", "Agent")}</label>
                            ...
                            {/* Payment Method */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("insurance.labels.paymentMethod", "Payment Method")} <span className="text-red-500">*</span></label>
                                <select name="paymentMethod" value={formData.paymentMethod} onChange={handleInputChange} className="mt-1 w-full p-2.5 border rounded-md dark:bg-gray-700 dark:text-white">
                                    <option value="">{t("insurance.placeholders.choosePaymentMethod", "Choose Payment Method")}</option>
                            ...
                            {/* Price On Customer */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("insurance.labels.priceOnCustomer", "Price on Customer")} <span className="text-red-500">*</span></label>
                                <input type="number" name="priceisOnTheCustomer" value={formData.priceisOnTheCustomer} onChange={handleInputChange} className="mt-1 w-full p-2.5 border rounded-md dark:bg-gray-700 dark:text-white" step="any" placeholder={t("insurance.placeholders.enterPrice", "Enter price")} />
                            </div>

                            {/* Paid Amount */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("insurance.labels.paidAmount", "Paid Amount")} <span className="text-red-500">*</span></label>
                                <input type="number" name="paidAmount" value={formData.paidAmount} onChange={handleInputChange} className="mt-1 w-full p-2.5 border rounded-md dark:bg-gray-700 dark:text-white" step="any" placeholder={t("insurance.placeholders.enterAmount", "Enter amount")} />
                            </div>

                            {/* Is Under 24 */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("insurance.labels.driverUnder24", "Driver Under 24?")}</label>
                        ...
                        {/* Attachments */}
                        <div className='mt-4'>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t("insurance.labels.attachments", "Attachments")} ({t("common.optional", "Optional")})</label>
                            {files.length > 0 && (
                            ...
                            <div onClick={handleBrowseClick} className={`relative flex cursor-pointer flex-col items-center justify-center h-40 border-2 border-dashed rounded-md transition-colors ${isDragging ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30" : "border-gray-300 hover:border-gray-400 dark:border-gray-600"}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
                                <div className="flex flex-col items-center justify-center space-y-2 text-center p-4">
                                    ...
                                    <p className="text-sm text-gray-600 dark:text-gray-400"><span className="font-semibold text-indigo-600 dark:text-indigo-400 hover:underline" onClick={(e) => { e.stopPropagation(); handleBrowseClick(); }}>{t("insurance.uploadPrompt", "Click to upload")}</span> {t("insurance.orDragDrop", "or drag and drop")}</p>
