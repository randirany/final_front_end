...
import { updateRoadService } from '../services/roadServiceApi';

const EditRoadServiceModal = ({ isOpen, onClose, onRoadServiceUpdated, roadService, companies }) => {
  ...
  useEffect(() => {
    if (isOpen && roadService) {
      setFormData({
        service_name: roadService.service_name || '',
        normal_price: roadService.normal_price || '',
        old_car_price: roadService.old_car_price || '',
        cutoff_year: roadService.cutoff_year || '2007',
        description: roadService.description || '',
        is_active: roadService.is_active !== undefined ? roadService.is_active : true
      });
      setErrors({});
    }
  }, [isOpen, roadService]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateRoadService(roadService._id, roadServiceData);

      toast.success(t('roadService.messages.updateSuccess', 'Road service updated successfully'));
      if (onRoadServiceUpdated) onRoadServiceUpdated();
      onClose();
    } catch (error) {
      console.error('Error updating road service:', error);
      toast.error(error.response?.data?.message || t('roadService.messages.updateError', 'Failed to update road service'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={handleBackdropClick}>
      <div className="w-full max-w-2xl bg-white rounded-lg shadow-xl dark:bg-navbarBack max-h-[95vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
          <h2 className="text-2xl font-semibold dark:text-white">{t('roadService.editRoadService', 'Edit Road Service')}</h2>
          <button
        ...
        <form className="flex-1 overflow-y-auto" onSubmit={handleSubmit}>
          <div className="p-6 space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium dark:text-gray-300 mb-1">
                {t('roadService.labels.company', 'Insurance Company')}
              </label>
              <div className="w-full p-2.5 border rounded-md dark:bg-gray-700 dark:border-gray-600 bg-gray-50 dark:bg-gray-800">
                <span className="text-gray-700 dark:text-gray-300">
                  {roadService?.company_id?.name || 'N/A'}
                </span>
              </div>
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {t('roadService.help.companyReadonly', 'Company cannot be changed after creation')}
              </p>
            </div>
          ...
          <div className="flex gap-3 justify-end px-6 py-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            ...
            <button
              ...
              {loading ? t('common.updating', 'Updating...') : t('roadService.buttons.update', 'Update')}
            </button>
          </div>
        </form>
      </div>
...
export default EditRoadServiceModal;
