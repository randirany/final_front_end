...
function InsuranceCompany() {
    ...
    const handlePrint = () => { const printWindow = window.open('', '_blank', 'height=600,width=800'); printWindow.document.write('<html><head><title>Insurance Companies Report</title>'); printWindow.document.write('<style>'); printWindow.document.write(` body { font-family: ${(language === 'ar' || language === 'he') ? 'Cairo, sans-serif' : 'Arial, sans-serif'}; direction: ${(language === 'ar' || language === 'he') ? 'rtl' : 'ltr'}; } table { border-collapse: collapse; width: 100%; margin-top: 20px; } th, td { border: 1px solid #ddd; padding: 8px; text-align: ${(language === 'ar' || language === 'he') ? 'right' : 'left'}; } th { background-color: #f2f2f2; } h1 { text-align: center; } @media print { .no-print { display: none; } } `); printWindow.document.write('</style></head><body>'); printWindow.document.write(`<h1>${t('insuranceCompany.printReportTitle', 'Insurance Companies Report')}</h1>`); printWindow.document.write('<table><thead><tr>'); tableHeaders.forEach(col => { if (col.key !== 'actions') { printWindow.document.write(`<th>${col.label}</th>`); } }); printWindow.document.write('</tr></thead><tbody>'); filteredCompanies.forEach(company => { printWindow.document.write('<tr>'); tableHeaders.forEach(col => { if (col.key !== 'actions') { const value = company[col.key] !== null && company[col.key] !== undefined ? company[col.key] : '-'; printWindow.document.write(`<td>${value}</td>`); } }); printWindow.document.write('</tr>'); }); printWindow.document.write('</tbody></table></body></html>'); printWindow.document.close(); printWindow.onload = function () { printWindow.focus(); printWindow.print(); printWindow.close(); }; };
                            ...
                            visibleCompanies.map((company) => (
                                <tr key={company.id} className="bg-[rgb(255,255,255)] dark:bg-navbarBack border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td className={`px-6 py-4 ${(language === 'ar' || language === 'he') ? 'text-left' : 'text-right'}`}>
                                        ...
                                        <Menu anchorEl={anchorEls[company.id]} open={Boolean(anchorEls[company.id])} onClose={() => handleMenuClose(company.id)} anchorOrigin={{ vertical: 'bottom', horizontal: (language === 'ar' || language === 'he') ? 'left' : 'right' }} transformOrigin={{ vertical: 'top', horizontal: (language === 'ar' || language === 'he') ? 'left' : 'right' }} MenuListProps={{ className: 'dark:bg-gray-800 dark:text-gray-200' }} >
